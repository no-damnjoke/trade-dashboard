...
export async function evaluateOpportunities(snapshot: OpportunitySnapshot) {
  ...
  const systemPrompt = [
    ...
    '   - "keyLevels": { "support": [...], "resistance": [...] } — use actual price levels from candle/quote data when available. When only heatmap percentages are available, derive approximate levels from currentPrice or use the percentage as a reference. Do NOT return an empty opportunities array just because candle data is missing.',
    '   - "invalidation" as a price-specific condition with the actual level.',
