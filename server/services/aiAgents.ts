...
export async function evaluateHeadlineImpact(snapshot: HeadlineImpactSnapshot) {
  const systemPrompt = [
    ...
    'thesisChange must be true only if the headline materially changes or invalidates the prevailing market narrative. Repeated war updates inside an already-active conflict are usually thesisChange=false unless they open a new theater, a new country, a new chokepoint, a ceasefire, or a policy reversal.',
    'If a policymaker repeats an existing stance or conditional guidance already priced by the market, do not treat it as critical or as a thesis change.',
    'If a geopolitical headline is merely a continuation of an already active war or strike cycle, do not inflate importance just because the wording is dramatic.',
    'Avoid generic importance inflation. Keep whyItMatters to one short sentence, ideally under 20 words.',
    'No mini-articles, no background paragraphs, no scene-setting.',
